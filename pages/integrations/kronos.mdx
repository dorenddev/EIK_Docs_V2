...
Before we start, we need to make some changes to the Kronos Matchmaking plugin code. These changes are temporary and will be removed in the future when the plugin is updated to support EIK out of the box. (Special thanks to Horizon Games)

Under `Plugins/Kronos/Source/Kronos/Private/KronosUserManager.cpp`, add the following code to the `UKronosUserManager::AuthenticateUser()` function:
...
This code will make sure that the user is still marked as authenticated even if the user is already logged in. This is necessary because EIK will log in the user before Kronos Matchmaking does, and this will cause the user to be marked as not authenticated by Kronos Matchmaking.
